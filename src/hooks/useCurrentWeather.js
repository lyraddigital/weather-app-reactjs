...
export const useCurrentWeather = () => {
    ...
    return {
        ...
        statistics: { 
            ...
            sunriseTime: weatherData?.current?.sunrise ? utcToZonedTime(weatherData.current.sunrise * 1000, weatherData.timezone): new Date(), 
            sunsetTime: weatherData?.current?.sunset ? utcToZonedTime(weatherData.current.sunset * 1000, weatherData.timezone): new Date(),
            localTime: weatherData?.current?.dt ? utcToZonedTime(weatherData.current.dt * 1000, weatherData.timezone): new Date()
        },
        ...
        forecast: weatherData.daily.map(d => ({
            date: d?.dt ? utcToZonedTime(d.dt * 1000, weatherData.timezone): new Date(),
